...
### Reconnection Consent Gate
- When a block expires, A is not automatically reconnected. A is shown a password confirmation screen before reconnecting.
- This is a consent gate — not a new login, not a session reset. It re-authenticates in place using supabase.auth.signInWithPassword with the current user's email and the entered password.
- The password is the signature. It confirms the reconnection is A's deliberate decision — not an accident, not an impulse, not a moment of weakness.
- Modeled on GitHub's repo deletion pattern and Meta's account deletion pattern: high-stakes irreversible action → require typed confirmation → proceed.
- If the password is wrong, show an inline error. Do not clear the input. Do not log the user out.
- A is always offered "Step away again" as an alternative — a link to /block/[username] to initiate a new block instead of reconnecting.
- B receives no notification that A's block has expired or that A has chosen to reconnect.
